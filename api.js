...
let { waitForBalance, accountCreate, getTotalReceived, send, receiveAllPending,
    refundAccount, getLatestTransaction, accountHistory, isAccountValid } = require('./lib/rai');
...
let { createTransaction, getTransaction, setTransactionStatus,
    processTransaction, refundTransaction, recoverAndRefundTransaction, recoverAndProcessTransaction } = require('./transaction');
...
app.post('/api/session', handler(async (req, res) => {
    ...
    let id = await createTransaction({ destination, amount, amount_rai, account, currency, privateKey, publicKey });
...
app.post('/api/session/:token/transfer', handler(async (req, res) => {
    ...
    let { account, destination, amount_rai } = await getTransaction(id);
...
app.get('/api/refund/:account', handler(async (req, res) => {
...
app.get('/api/process/:account', handler(async (req, res) => {
